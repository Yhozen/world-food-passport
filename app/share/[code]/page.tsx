...
import { JoinSharedVisit } from "./_components/join-shared-visit";
...
export default async function SharedVisitPage({ params }: SharedVisitPageProps) {
  ...
  const data = await queryClient.fetchQuery(
    trpc.sharedVisits.getByCode.queryOptions({ shareCode: code }),
  );
      ...
      <div className="mx-auto w-full max-w-4xl px-6 py-10">
        ...
        {!isMember ? (
          <div className="mt-6 rounded-3xl border border-amber-200 bg-amber-50/80 p-6 shadow-sm">
            ...
            <div className="mt-4">
              <JoinSharedVisit shareCode={code} />
            </div>
          </div>
        ) : (
          <SharedVisitGallery
